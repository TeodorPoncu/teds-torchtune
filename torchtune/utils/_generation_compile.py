...
def generate_next_token_no_sample(
    model: TransformerDecoder,
    input_pos: torch.Tensor,
    x: torch.Tensor,
    temperature: float = 1.0,
    ...
    """Generates the next token logits without sampling."""
    # we add this change in order to make it play ball with the torch.compile
    logits = model(x, input_pos=input_pos)[:, -1]
    return logits
...
def generate(
    ...
    *,
    ...
    for step in range(max_generated_tokens - 1):
        ...
        # NOTE: THIS ASSUMES WE'RE WORKING WITH `torch.compile`
        # NOTE: DO NOT USE THIS FUNCTION WITHOUT `torch.compile`     
        token_logits = custom_generate_next_token(
            model,
