...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def test(self) -> None:
        ...
        for dset_name, dset_loader in self._test_loaders.items():
            ...
            pbar = tqdm(total=len(dset_loader), disable=not (rank == 0), desc=f"Testing {dset_name}")
            
            for idx, batch in enumerate(dset_loader):
                tokens    = batch["tokens"]
                labels    = batch["labels"]
            ...
            # invoke barrier to ensure all processes have finished testing
            torch.distributed.barrier()

            # reduce loss on rank 0 since we are logging only on rank 0
